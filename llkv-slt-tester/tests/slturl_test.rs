...
fn test_slturl_file_not_found() {
    ...
    assert!(
        result.is_err(),
        "Expected error for non-existent .slturl file"
    );
...
fn test_slturl_invalid_url() {
    ...
    temp_file
        .write_all(b"https://invalid.example.com/nonexistent/test.slt")
    ...
    let err_msg = format!("{:?}", result.err().unwrap());
    assert!(
        err_msg.contains("failed to fetch SLT URL")
            || err_msg.contains("dns error")
            || err_msg.contains("error sending request"),
        "Expected network/fetch error, got: {}",
        err_msg
    );
}
