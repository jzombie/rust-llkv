...
LLKV is an experimental SQL database built as a Rust workspace. It layers [Apache Arrow](https://arrow.apache.org/) buffers, a streaming execution engine, and MVCC transaction management on top of generic key-value pagers.
...
- Synchronous execution is the default. Hot paths lean on [Rayon](https://crates.io/crates/rayon) work-stealing and [Crossbeam](https://crates.io/crates/crossbeam) coordination instead of a pervasive async runtime so individual queries can keep scheduler overhead low, yet the engine still embeds cleanly inside Tokio—our SQL Logic Test runner spins up a Tokio runtime to simulate concurrent connections.
...
- **SQL Interface** ([`llkv-sql`](./llkv-sql/)): Parses SQL, normalizes dialect quirks, batches `INSERT` workloads, and exposes `SqlEngine` as the main entry point.
- **Query Planning** ([`llkv-plan`](./llkv-plan/), [`llkv-expr`](./llkv-expr/)): Translates parsed SQL into typed plans, models subqueries and scalar expressions, and keeps correlation plumbing shared.
- **Runtime and Orchestration** ([`llkv-runtime`](./llkv-runtime/), [`llkv-transaction`](./llkv-transaction/)): Manages sessions, namespaces, MVCC snapshots, and coordinates plan execution across storage and execution layers.
- **Query Execution** ([`llkv-executor`](./llkv-executor/), [`llkv-aggregate`](./llkv-aggregate/), [`llkv-join`](./llkv-join/)): Streams Arrow batches through projection, filtering, aggregation, and join pipelines without buffering whole result sets.
...
2. Plans are built by [`llkv-plan`](./llkv-plan/), which annotates correlated subqueries, scalar programs, and DML metadata.
3. [`llkv-runtime`](./llkv-runtime/) acquires a transaction snapshot, injects MVCC metadata, and dispatches plans to the appropriate subsystem.
...
- [`llkv-column-map`](./llkv-column-map/) persists column chunks as Arrow-serialized blobs keyed by pager-managed physical IDs.
...
- [`llkv-transaction`](./llkv-transaction/) allocates monotonic transaction IDs, tracks commits, and enforces snapshot visibility during scans and DML replay.
...
- SQL Logic Tests: [llkv-slt-tester](./llkv-slt-tester/) wraps [sqllogictest](https://sqlite.org/sqllogictest/doc/trunk/about.wiki) suites with `LlkvSltRunner`, pointer (`.slturl`) support, and optional query statistics (`LLKV_SLT_STATS=1`).
- CI: [GitHub Actions workflows](.github/workflows/) cover linting (`cargo fmt`, `clippy`, `cargo doc`, `cargo deny`, `cargo audit`) and run the full test matrix on Linux, macOS, and Windows.
- Benchmarks: Criterion benchmarks run on a self-hosted macOS ARM64 runner, with [CodSpeed][codspeed-page] ingesting results for trend tracking.
