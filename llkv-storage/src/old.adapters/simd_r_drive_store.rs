...
impl KeyValueStore<StorageKey> for SimdRDriveStore {
    fn insert_batch(&mut self, items: FxHashMap<StorageKey, Vec<u8>>) -> io::Result<()> {
        ...
        self.inner.batch_write_with_key_hashes(entries, false)?;

        Ok(())
    }

    // *** ZERO-COPY: return EntryHandle, not Vec<u8> ***
    fn get_batch(&self, keys: &[StorageKey]) -> io::Result<FxHashMap<StorageKey, EntryHandle>> {
        let mut out: FxHashMap<StorageKey, EntryHandle> =
            FxHashMap::with_capacity_and_hasher(keys.len(), Default::default());

        let handles = self.inner.batch_read_hashed_keys(keys, None)?;
    ...
    fn remove_batch(&mut self, keys: &[StorageKey]) -> io::Result<()> {
        self.inner.batch_delete_key_hashes(keys)?;

        Ok(())
    }

    fn contains_key(&self, key: &StorageKey) -> io::Result<bool> {
        self.inner.exists_with_key_hash(key.to_owned())
    }

    // ---------- TEST-UTILS (feature-gated) ----------
    #[cfg(feature = "debug")]
    fn keys(&self) -> io::Result<Vec<StorageKey>> {
        Ok(self
            .inner
            .iter_entries()
            .map(|e| e.key_hash())
    ...
    #[cfg(feature = "debug")]
    fn inspect_all(&self) -> io::Result<Vec<(StorageKey, usize)>> {
        Ok(self
            .inner
            .iter_entries()
    ...
    #[cfg(feature = "debug")]
    fn inspect_subset(&self, keys: &[StorageKey]) -> io::Result<Vec<(StorageKey, usize)>> {
        let handles = self
            .inner
...
mod tests {
    ...
    fn crud_roundtrip_and_contains() {
        ...
        store.insert_batch(to_write.clone()).unwrap();

        // After insert: all keys exist.
        for k in [1u64, 2, 3] {
            assert!(store.contains_key(&k).unwrap(), "missing key after insert");
        }

        // Read back via zero-copy EntryHandle.
        let got = store.get_batch(&[1u64, 2, 3]).unwrap();
        ...
        let after_del = store.get_batch(&[1u64, 2u64, 3u64]).unwrap();
    ...
    fn get_batch_skips_missing_and_iter_handles_matches_set() {
        ...
        let got = store.get_batch(&[9u64, 10, 11, 12, 13]).unwrap();
    ...
    fn persistence_across_reopen_and_delete_persists() {
...

        {
            ...
            assert!(store.contains_key(&1001).unwrap());
            assert!(store.contains_key(&1002).unwrap());
            let got = store.get_batch(&[1001u64, 1002u64]).unwrap();
            let got_bytes = bytes_map_from_handles(&got);
        ...
        {
            let mut store = SimdRDriveStore::new(&path);
            store.remove_batch(&[1001u64]).unwrap();
            assert!(!store.contains_key(&1001).unwrap());
            assert!(store.contains_key(&1002).unwrap());
        }
    ...
    fn inspect_all_and_keys_cover_and_size_correctly() {
        ...
        let mut all = store.inspect_all().unwrap();
    ...
    fn inspect_subset_returns_only_requested_live_keys_with_sizes() {
        ...
        let mut subset = store.inspect_subset(&[3000u64, 3001u64, 3002u64]).unwrap();
        ...
        store.remove_batch(&[3001u64]).unwrap();
        let mut subset2 = store.inspect_subset(&[3001u64, 3002u64]).unwrap();
