...
fn log_pool_size_once() {
    ...
    LOGGED.get_or_init(|| {
        ...
        tracing::debug!(
            "[llkv-threading] Rayon pool initialized with {count} threads (LLKV_MAX_THREADS={})",
...
///
/// This helper ensures all parallel work within the workspace shares the same
...
mod tests {
    ...
    fn env_override_zero_defaults() {
        ...
        // Unsafe is fine for test-only env mutation in a single-threaded context.
        unsafe {
