...
use llkv_column_map::column_store::{Bounds, ColumnStore, Direction, ScanOptions};
...
fn value_sidecar_orders_and_bounds() {
    ...
    for i in 0..n {
        vals[i] = ((i as u64).wrapping_mul(1_103_515_245).rotate_left(7)) ^ 0xA5A5_5A5A_A5A5_5A5A;
    }

    // Build value-order permutation (ascending by value).
    let mut perm: Vec<u32> = (0..n as u32).collect();
    perm.sort_by_key(|&i| vals[i as usize]);

    // Append data chunk + sidecar.
    let (_pk, _ppk) = store.append_u64_chunk_with_value_perm(fid, &vals, &perm);
    ...
    // Cross-check against sum on original stripe.
    ...
    if let Some(iter) = store.scan_u64_value_order(fid, opts_b) {
        ...
        panic!("value-order scan not available");
    }
    assert!(total > 0);
    assert!(seen_min >= lo);
    assert!(seen_max <= hi);
