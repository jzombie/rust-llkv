...
use arrow::array::{UInt64Array, Int32Array};
...
fn seed_u64_perm(n: usize, seed: u64) -> (ColumnStore<MemPager>, LogicalFieldId, LogicalFieldId, Vec<u64>, Vec<usize>) {
    ...
    for (i, &v) in vals.iter().enumerate() { pos_of_val[v as usize] = i; }
...
fn row_ids_unsorted_and_sorted_paths_u64() {
    ...
    let mut u = UnsortedCheck { r: Vec::with_capacity(N), v: Vec::with_capacity(N) };
    store.scan(fid, ScanOptions { sorted: false, reverse: false, with_row_ids: true, row_id_field: Some(rid_fid) }, &mut u).unwrap();
    assert_eq!(u.r.len(), N);
    ...
    struct SortedAsc { r: Vec<u64>, v: Vec<u64> }
    impl PrimitiveSortedWithRowIdsVisitor for SortedAsc {
        fn u64_run_with_rids(&mut self, v: &UInt64Array, r: &UInt64Array, s: usize, l: usize) {
            let e = s + l; for i in s..e { self.v.push(v.value(i)); self.r.push(r.value(i)); }
        }
    }
    ...
    let mut sa = SortedAsc { r: Vec::with_capacity(N), v: Vec::with_capacity(N) };
    store.scan(fid, ScanOptions { sorted: true, reverse: false, with_row_ids: true, row_id_field: Some(rid_fid) }, &mut sa).unwrap();
    assert!(sa.v.len() == N && sa.r.len() == N);
    assert!(sa.v.windows(2).all(|w| w[0] <= w[1]));
    for i in 0..N { assert_eq!(sa.v[i] as usize, i); assert_eq!(sa.r[i] as usize, pos[i]); }

    // Sorted descending with row ids: values N-1..0; rids equal to position of each value
    struct SortedDesc { r: Vec<u64>, v: Vec<u64> }
    impl PrimitiveSortedWithRowIdsVisitor for SortedDesc {
        fn u64_run_with_rids(&mut self, v: &UInt64Array, r: &UInt64Array, s: usize, l: usize) {
            let mut i = s + l; while i > s { i -= 1; self.v.push(v.value(i)); self.r.push(r.value(i)); }
        }
    }
    ...
    let mut sd = SortedDesc { r: Vec::with_capacity(N), v: Vec::with_capacity(N) };
    store.scan(fid, ScanOptions { sorted: true, reverse: true, with_row_ids: true, row_id_field: Some(rid_fid) }, &mut sd).unwrap();
    assert!(sd.v.windows(2).all(|w| w[0] >= w[1]));
    for i in 0..N { let v = sd.v[i] as usize; assert_eq!(sd.r[i] as usize, pos[v]); }

    // Builder with range + row ids: subset correctness
    struct RangeCollect { r: Vec<u64>, v: Vec<u64> }
    impl PrimitiveSortedWithRowIdsVisitor for RangeCollect {
        fn u64_run_with_rids(&mut self, v: &UInt64Array, r: &UInt64Array, s: usize, l: usize) { let e=s+l; for i in s..e { self.v.push(v.value(i)); self.r.push(r.value(i)); } }
    }
    ...
    let mut rc = RangeCollect { r: Vec::new(), v: Vec::new() };
    ...
    for i in 0..rc.v.len() { let v = rc.v[i] as usize; assert_eq!(rc.r[i] as usize, pos[v]); }
}
