...
use llkv_column_map::store::{ColumnStore, FIELD_ID_META_KEY, GatherNullPolicy, ROW_ID_COLUMN_NAME};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    md_x.insert(FIELD_ID_META_KEY.to_string(), u64::from(field_id_x).to_string());
    
    let mut md_y = HashMap::new();
    md_y.insert(FIELD_ID_META_KEY.to_string(), u64::from(field_id_y).to_string());
    ...
    println!("=== INSERT 3 rows ===");
    {
        ...
        let y = Arc::new(StringArray::from(vec![Some("true"), Some("false"), Some("NULL")]));
    ...
    let batch = store.gather_rows(&[field_id_x, field_id_y], &[1, 2, 3], GatherNullPolicy::IncludeNulls)?;
    ...
    println!("\n=== UPDATE all rows SET x=99 ===");
    {
        ...
        let y = Arc::new(StringArray::from(vec![Some("unknown"), Some("false"), Some("NULL")]));
        
        let batch = RecordBatch::try_new(schema.clone(), vec![rowid, x, y])?;
        println!("Appending: rowid={:?}, x={:?}, y={:?}", batch.column(0), batch.column(1), batch.column(2));
        store.append(&batch)?;
    }
    ...
    println!("\n=== VERIFICATION ===");
    let batch = store.gather_rows(&[field_id_x, field_id_y], &[1, 2, 3], GatherNullPolicy::IncludeNulls)?;
    let x_vals = batch.column(0).as_any().downcast_ref::<Int64Array>().unwrap();
    let y_vals = batch.column(1).as_any().downcast_ref::<StringArray>().unwrap();
    
    println!("Final state:");
    for i in 0..3 {
        println!("  Row {}: x={:?}, y={:?}", 
            i+1,
            if x_vals.is_null(i) { "NULL".to_string() } else { x_vals.value(i).to_string() },
            if y_vals.is_null(i) { "NULL".to_string() } else { y_vals.value(i).to_string() });
    }
    ...
    if x_vals.is_null(2) {
        ...
        println!("Got: x=NULL, y={:?}", if y_vals.is_null(2) { "NULL" } else { y_vals.value(2) });
        std::process::exit(1);
    }
    
    if x_vals.value(2) != 99 {
        println!("\n❌ Row 3 x has wrong value: {} (expected 99)", x_vals.value(2));
        std::process::exit(1);
    }
