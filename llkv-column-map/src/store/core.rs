    ...
    P: Pager<Blob = EntryHandle> + Send + Sync,
{
        ...
        F: FnMut(ProjectionBatch) -> Result<()>,
    {
        ...
        // Ensure row ids accompany every batch; the visitor depends on them for alignment.
...
/// values for a contiguous slice of the column-arena backing storage.
    ...
    F: FnMut(ProjectionBatch) -> Result<()>,
{
    ...
    fn emit_null_run(&mut self, row_ids: &UInt64Array, start: usize, len: usize) {
        let slice = row_ids.slice(start, len);
        let rid = Arc::new(slice);
